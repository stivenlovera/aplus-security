var ZtSliderAdmin = new function(){
	...
	var initSaveSliderButton = function(ajaxAction){
		...
		jQuery("#button_save_slider,#button_save_slider_t").click(function(){
			//collect data
			var data = {
				params: ZtSliderSettings.getSettingsObject("form_slider_params"),
				main: ZtSliderSettings.getSettingsObject("form_slider_main"),
				template: jQuery('#revslider_template').val() //determinate if we are a template slider or not
			};
			
			//add slider id to the data
			if(ajaxAction == "slider.updateslider"){
	...
	var updateShortcode = function(){
		...
		var shortcode = '[zt_layerslider alias="'+alias+'"]';
		if(alias == "")
	...
	**/
	...
	this.initAddSliderView = function(){
		...
		jQuery("#title").focus();
		initSaveSliderButton("slider.createslider");
	...
	this.initEditSliderView = function(){
		...
		initSaveSliderButton("slider.updateslider");
		...
		jQuery("#button_delete_slider, #button_delete_slider_t").click(function(){
			...
			UniteAdminRev.ajaxRequest("slider.deleteslider" ,data);
		});
		...
		jQuery("#button_export_slider").click(function(){
			...
			var urlAjaxExport = rev_lang.root+"&task=slider.exportslider&dummy="+useDummy;
	...
	t.updateImageChange = function (urlImage) {
	...
	t.updateThumbnailChange= function(urlImage){
		//update input:
		var settingID = jQuery(".button-image-select").attr('id').replace("_button","");
		jQuery("#"+settingID).val(urlImage);

		//update preview image:
		jQuery("#" + settingID + "_button_preview").html('<div style="width:100px;height:70px;background:url(\''+urlImage+'\'); background-position:center center; background-size:cover;"></div>');
	...
	this.initSlidersListView = function(){
			...
			jQuery("#dialog_import_template_slider_from").dialog({
				...
				buttons:{
					...
					"Online":function(){
						...
						// 	jQuery(this).dialog("close");
						// }else{
							alert('We are developing this functionality');
						// }
					}
				}
			});
			...
			jQuery('#dialog_duplicate_slider').dialog({
				...
				buttons:{
					...
					'Import':function(){
						...
						UniteAdminRev.ajaxRequest('slider.duplicateslider', {sliderid:slider_id,title:jQuery('#rs-duplicate-animation').val()}, function(response){
							jQuery('#close-template').click();
						});
					}
				}
		...
		jQuery(".button_delete_slider").click(function(){
			...
			UniteAdminRev.ajaxRequest("slider.deleteslider" ,{sliderid:sliderID}, function(){
			...
			jQuery('#dialog_duplicate_slider').dialog({
				...
				buttons:{
					...
					'Duplicate':function(){
						...
						UniteAdminRev.ajaxRequest('slider.duplicateslider' ,{sliderid:sliderID,title:jQuery('#rs-duplicate-animation').val()}, function(response){
		...
		jQuery(".rev-toogle-fav").click(function(){
			...
			UniteAdminRev.ajaxRequest('slider.togglefavourite' ,{id:sliderID},function(){
		...
		jQuery('body').on('click', '.rs-embed-slider', function(){
			...
			jQuery('.rs-dialog-embed-slider').find('.rs-example-alias-1').text('[zt_layerslider alias="'+use_alias+'"]');
	...
	t.saveEditSlide = function(slideID, isDemo){
		...
		data.params = ZtSliderSettings.getSettingsObject("form_slide_params");
		...
		if(!isDemo){
			...
			UniteAdminRev.ajaxRequest("slider.updateslide", data);
		}else{
			UniteAdminRev.setAjaxHideButtonID("button_save_static_slide,button_save_static_slide-tb");
			UniteAdminRev.setAjaxLoaderID("loader_update");
			UniteAdminRev.setSuccessMessageID("update_slide_success");
			UniteAdminRev.ajaxRequest("slider.updatestaticslide", data);
		}
	}
	...
	this.initEditSlideView = function(slideID,sliderID,is_static){
		...
		jQuery("#link_add_slide").click(function(){
			...
			UniteAdminRev.ajaxRequest("slider.addslidefromslideview", data);
		});
		...
		jQuery('body').on('click', '.add_template_slide_item, .add_user_template_slide_item', function(){			
			if(confirm(rev_lang.unsaved_data_will_be_lost_proceed)){
				...
				UniteAdminRev.ajaxRequest('slider.copyslidetoslider', data, function(){
					jQuery('#close-template').click();
				});
			}
		});
		...
		jQuery('body').on('click', '.slide-remove', function(){
			...
			var do_request = (curSlideID == slideID) ? 'slider.deleteslide' : 'slider.deleteslidestay';
		...
		jQuery('body').on('click', '.slide-published.pubclickable, .slide-unpublished.pubclickable', function(){
			...
			UniteAdminRev.ajaxRequest("slider.toggleslidestate" ,data,function(response){
		...
		jQuery('body').on('click', '.slide-duplicate', function(){
			...
			var data = {slideID:slideID,sliderID:sliderID};
			UniteAdminRev.ajaxRequest("slider.duplicateslidestay", data, function(response){
				objButton.show();
				...
				if(response.success == true){
					...
					if(ahref == 'javascript:void(0)'){
						ahref = 'index.php?option=com_zt_layerslider&view=slides&id='+ response.id;
					}else{
		...
		jQuery('body').on('click', '.slide-moveto', function(){
			...
			buttons[textUpdate] = function(){
				...
				UniteAdminRev.ajaxRequest("slider.copymoveslidestay", data, function(response){
					objButton.show();
		...
		jQuery('body').on('click', '.slide-add-as-template', function(){
			...
			if(confirm(rev_lang.unsaved_changes_will_not_be_added)){
				...
				if (title != null) {
					...
					UniteAdminRev.ajaxRequest('slider.addslidetotemplate', data, function(response){
						objButton.show();
		...
		ZtSliderSettings.onoffStatus(jQuery('input[name="hideslideonmobile"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="stream_do_cover"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="stream_do_cover_both"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="kenburn_effect"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="save_performance"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="video_force_cover"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="video_nextslide"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="video_allowfullscreen"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="video_force_rewind"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="video_mute"]'));
		ZtSliderSettings.onoffStatus(jQuery('input[name="thumb_for_admin"]'));
	...
	var openPreviewSlideDialog = function(slideID,useParams){
		...
		if(useParams == true){
			objData.params = ZtSliderSettings.getSettingsObject("form_slide_params"),
	...
	var initGlobalCssEditor = function(){
		...
		jQuery('#button_edit_css_global').click(function(){
			...
			UniteAdminRev.ajaxRequest("slider.getstaticcss","",function(response){
				...
				if(g_codemirrorCssStatic != null)
					...
					setTimeout('ZtSliderAdmin.setCodeMirrorStaticEditor()',500);
				}
			});

			UniteAdminRev.ajaxRequest("slider.getdynamiccss","",function(response){
				var cssData = response.data;
				...
				if(g_codemirrorCssDynamic != null)
					...
					setTimeout('ZtSliderAdmin.setCodeMirrorDynamicEditor()',500);
				}
			});
			...
			jQuery("#css_static_editor_wrap").dialog({
				...
				buttons:{
					Save:function(){
						...
						UniteAdminRev.ajaxRequest("slider.updatestaticcss",data,function(response){
							if(g_codemirrorCssStatic != null)
						...
						//	setTimeout('UniteAdminRev.loadCssFile(ZtSliderAdmin.getUrlStaticCssCaptions(),"rs-plugin-static-css");',1000);
